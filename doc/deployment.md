...
候选详情会从 Bilibili 读取播放量、评论数，并通过公开的 UP 关系统计接口读取粉丝数（相同 UP 在进程内缓存 6 小时）。UP 低于 `minimum_uploader_followers` 时直接拒绝，手工信任也不能绕过；视频低于 `minimum_auto_confirm_video_views` 时只进入人工审核。设为 `0` 可以关闭对应门槛。旧版短暂使用过的配置名 `minimum_auto_confirm_uploader_followers` 仍兼容，但建议改成新名称。`泄露`、`泄漏`、`偷跑`、`流出`、`枪版`、`盗录` 等标题信号始终硬拒绝。
...
尚未进入 `bangumi-data` 的未上映作品必须填写 Bangumi ID。AniPulse 会读取 Bangumi 条目的正式标题、中文名、首播日期和类型，再与 AnimeSchedule 的日文、罗马字、英文标题及别名进行高置信匹配；成功后把 AnimeSchedule route 持久化，后续直接按 route 同步。匹配器会分别核对“第几季”和“第几部分”，支持 `第2期/第二季/Season 2` 以及 `第2クール/第2部分/Part 2/Cour 2` 等写法，避免把 `Season 2 Part 1` 的两个编号混为一谈。为兼容网络先行与电视首播日期不同，标题、季度和类型吻合时允许两边日期在 `max_stream_offset_days`（默认 14 天）内偏移，并把该偏移应用到后续单集日期；超过阈值的跨年或错误季度仍会拒绝。若已有 AniList Media ID，可在网页高级选项或 CLI 的 `--anilist-id` 中填写，作为 AnimeSchedule 的精确检索键；AniPulse 不再请求 AniList API。若仍没有唯一结果，核对 AnimeSchedule 条目 URL 末尾的 route，再填写 `--anime-schedule-route`。这两个辅助字段都不能脱离 Bangumi ID 单独填写。AnimeSchedule 没有精确时刻、但 Bangumi 已有开播日期时可以正常添加：页面显示“仅开播日期，时间待公布”，系统从该日期开始检查并每日同步；精确周排期发布后会自动校准。若连开播日期都没有，才需要暂用手工排期。作品后来进入 `bangumi-data` 时不会立即丢弃已绑定 route：只有目录已经提供可用的受信网络平台时段才切回目录；若目录只有默认电视时段，仍优先采用 AnimeSchedule 的网络排期，接口临时失败时才保留目录默认值并显示告警。
