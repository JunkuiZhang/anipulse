...
1. **监控中**：正常同步排期并检查新集；
2. **已播完 · 待看完**：从详情页手工标记，停止查找下一集，但保留已经确认的往集视频；
3. **已归档**：只在用户勾选“已经实际看完本季”并完成二次确认后进入。

“第 12 集已经发布”不代表用户已经看完，因此 AniPulse 不会自动归档。未来即使增加自动完结识别，也只允许进入“已播完 · 待看完”。误标播完时，可以在详情页指定下一集并恢复监控。

归档会在一个事务中清理 Episode、候选、通知、逐集视频来源、UP 信任和非运行中的后台任务，保留标题、别名、Bangumi 绑定、服务器本地封面、总集数、简介/观看记录和归档时间。归档收藏在“追番 → 归档收藏”中查看；永久删除收藏仍需标题确认。数据库迁移不会自动归档任何现有条目。
...
- “泄露/偷跑/盗录”等高风险标题会直接拒绝；低粉 UP、低播放视频或详情元数据不完整时只进入人工审核，不能参与自动共识；
