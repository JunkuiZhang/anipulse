...
归档会先保存一份很小的观看统计快照，再清理 Episode、候选、通知、逐集视频来源、UP 信任和非运行中的后台任务。快照包含开始追踪、首次确认、看完与归档时间，以及按“总集数 × 单集时长范围中值”计算的约合观看时长；不会因为 B 站合集或异常视频时长而失真。标题、别名、Bangumi 绑定、服务器本地封面和总集数也会继续保留。

归档详情页可以随时填写或修改 1–10 分的个人评分、一句话短评、标签和观后感；有 Bangumi ID 时，归档动作还会通过后台任务缓存 Bangumi 的公开评分、评分人数与排名，详情页可手动刷新。该请求复用 `schedule.bangumi_api_base_url`：配置为 Cloudflare Worker 时会经 Worker，配置为 `https://api.bgm.tv` 时则由服务器直连。归档列表会并列显示个人评分与公开评分，并显示开始观看、看完日期及观看历时。

升级前已经归档的收藏，其逐集数据无法恢复，因此不会伪造“开始观看”日期，而会改为显示“开始追踪”和“追踪历时”；首次确认时间也会标为“旧归档未保留”。总集数、片长估算和归档日期仍会自动补齐。归档收藏在“追番 → 归档收藏”中查看；永久删除收藏仍需标题确认。升级数据库不会自动归档任何现有条目，只有明确点击“已观看”或手工确认归档才会触发。
...
候选列表中的“屏蔽此 UP”只对当前番剧生效：它会在一个事务中记录屏蔽状态，并拒绝该 UP 在此番剧下已有的全部待审核候选；以后检测到的候选也会自动排除。操作完成后页面会显示结果提示。信任按钮分为“仅信任此番”和“全局信任”；某部番单独设置的屏蔽始终优先于全局信任。

导航栏的“规则”页提供两类可编辑规则：

- 屏蔽词是全局规则，匹配候选的标题、简介或标签后硬拒绝；新建或修改后从下一次检查开始生效，当前仍在列表中的候选可以先手工拒绝或点“立即检查”重新评估；
- 全局信任 UP 对所有追番生效，适合稳定上传多部番剧的 UP，可以添加、修改 UID/显示名称或移除；
- 按番剧信任只影响指定番剧，可以添加、改 UID/显示名称、换所属番剧、移除或升级为全局信任。升级会清理同一 UID 的重复按番信任，但保留历史信誉计数和按番屏蔽；
- 两种信任都不会绕过番名、集数、时长、低粉丝门槛、合集识别和屏蔽词等硬性规则。
