...
export function resolveRoute(url) {
  ...
  const subject = url.pathname.match(/^\/bangumi\/v0\/subjects\/([1-9]\d*)$/);
  if (subject && [...url.searchParams].length === 0) {
    return {
      ...ROUTES.subject,
      accept: "application/json",
      upstreamUrl: new URL(`/v0/subjects/${subject[1]}`, BANGUMI_API_ORIGIN).toString(),
    };
  }
